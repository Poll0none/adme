...
RightGroupBox:AddInput('eggbuy', {Default = 'Number of eggs',Numeric = true,Finished = false,Text = 'How many?',Tooltip = 'Numbers Only',Placeholder = 'WalkSpeed'})
Options.eggbuy:OnChanged(function(State)
    Amount = State
    spawn(function()
        while Amount and wait() do
            pcall(function()
                for i = Amount, 1, -1 do
                    wait(1)
                    ReplicatedStorage.API["ShopAPI/BuyItem"]:InvokeServer("pets",Egg,{})
...
LeftGroupBox2:AddDropdown('Method', {Values = {"Pets","Gifts","GrownPets-1","Eggs"}, Default = 1, Multi = false, Text = 'Transfer Items', Tooltip = '', Callback = function(A)
    Petsd = A
end})
