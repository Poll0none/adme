...
if game.PlaceId == 920587237 then
    ...
    local LocationPassive = nil
    for i, v in pairs(getgc()) do
        if type(v) == "function" then
            if getfenv(v).script == ReplicatedStorage.ClientModules.Core.InteriorsM.InteriorsM then
                if table.find(getconstants(v), "AdoptAPI/SendPassiveDoorEnter") then
                    LocationPassive = v
                    break
                end
            end
    ...
    local SetLocation = function(A, B, C, passive)
        local O = get_thread_identity()
        set_thread_identity(2)
        if passive then
            LocationPassive(A, B, C)
        else
            Location(A, B, C)
        end
        set_thread_identity(O)
    ...
    local GoToStore = function(Name, spawn_cframe)
        ...
        if spawn_cframe then
            local passive = true
            SetLocation(Name, "MainDoor", {["spawn_cframe"] = spawn_cframe}, passive)
        else
...
local GoToMiniGame = eventTab:CreateToggle({
...
local GoToEventShop = eventTab:CreateToggle({
    Name = "Go-To Event Shop",
    CurrentValue = false,
    Flag = "EventShop", -- A flag is the identifier for the configuration file, make sure every element has a different flag if you're using configuration saving to ensure no overlaps
    Callback = function(State)
        spawn_cframe = CFrame.new(2.21142578, 4.01519012, -1.56188965, -0.580787539, -0.450501442, 0.678036988, 0.00125139544, 0.832417786, 0.554147065, -0.814054191, 0.322690189, -0.482894152)
        GoToStore('Halloween2023Shop', spawn_cframe)
    end,
})
